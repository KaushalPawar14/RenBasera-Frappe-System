	...
	<div class="page">
		...
		<div v-else>
			...
			<div class="section">
				...
				<div
					v-if="!challan.center_list || challan.center_list.length === 0"
					class="empty-table"
				>
					No centers
				</div>
				<div v-else class="data-table">
					...
					<div class="table-row" v-for="(center, i) in challan.center_list" :key="i">
						<span class="col-lg item-name">{{
							center.name1 || center.center_name || "—"
						}}</span>
			...
			<div class="section" v-else>
				<div class="section-title">⚡ Actions</div>
				<div class="no-action">
					<span v-if="challan.workflow_state === 'Completed'"
						>🏁 This challan is completed.</span
					>
...
function getStatusClass(status) {
	const map = {
		Draft: "hero-draft",
		"Pending Security Check": "hero-pending",
		"Security Verified": "hero-verified",
		Dispatched: "hero-dispatched",
...
const availableActions = computed(() => {
	const state = challan.value.workflow_state || "Draft";
	const actions = [];
	if (state === "Draft") {
		if (auth.hasRole("TSF Distribution Manager") || auth.hasRole("TSF Admin")) {
			actions.push({
				label: "Submit for Security",
				icon: "🔐",
				style: "warning",
				desc: "Send to security checkpoint",
				workflow_action: "Submit for Security",
			});
			actions.push({
				label: "Cancel",
				icon: "❌",
				style: "danger",
				desc: "Cancel this challan",
				workflow_action: "Cancel",
			});
		}
	}
	if (state === "Security Verified") {
		if (auth.hasRole("TSF Distribution Manager") || auth.hasRole("TSF Admin")) {
			actions.push({
				label: "Dispatch",
				icon: "🚚",
				style: "primary",
				desc: "Send vehicle on route",
				workflow_action: "Dispatch",
			});
		}
	}
	if (state === "Dispatched") {
		if (auth.hasRole("TSF Driver")) {
			actions.push({
				label: "Mark Complete",
				icon: "🏁",
				style: "success",
				desc: "All deliveries done",
...
async function triggerAction(action) {
	...
	actionSuccess.value = "";
	try {
		const res = await apiPost("/api/method/frappe.model.workflow.apply_workflow", {
...
async function fetchChallan() {
	loading.value = true;
	try {
		const res = await apiGet(`/api/resource/TSF Dispatch Challan/${route.params.name}`);
		challan.value = res.data || {};
	} catch (e) {
		console.error(e);
	}
	loading.value = false;
