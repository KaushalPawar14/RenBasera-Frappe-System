...
// Import standard Axios instance
...
async function fetchChallanAndVehicle(challanRef) {
	...
	try {
		// Standard Resource GET for specific Document
		const challanRes = await api.get(`/api/resource/TSF Dispatch Challan/${challanRef}`);
		const cd = challanRes.data?.data || {};
		...
		if (cd.vehicle) {
			const vehicleRes = await api.get(`/api/resource/TSF Vehicle/${cd.vehicle}`);
			const vd = vehicleRes.data?.data || {};
...
async function fetchData() {
	loading.value = true;
	try {
		// Standard Resource List GET using params
		const allRes = await api.get("/api/resource/TSF Security Check", {
			params: {
...
async function triggerAction(action) {
	...
	try {
		...
		// Standard Resource PUT to update an existing DocType
		// Note: If Frappe enforces strict Workflow transitions here,
		// you might need to use frappe.model.workflow.apply_workflow instead of a standard PUT.
		const res = await api.put(`/api/resource/TSF Security Check/${selectedCheck.value.name}`, {
		...
		saveError.value = e.response?.data?.exception || e.message;
	} finally {
		saving.value = false;
	}
}
