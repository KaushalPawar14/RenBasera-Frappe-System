  ...
  <div class="page">
    ...
    <div class="content">
      ...
      <div class="card">
        ...
        <button class="login-btn" @click="handleLogin" :disabled="state.isLoading || loading">
          <span v-if="state.isLoading || loading" class="btn-spinner"></span>
...
// Use the native composable instead of a Pinia store
...
async function handleLogin() {
  ...
  try {
    ...
    // Call the login function from our composable
    const success = await login(usr.value, pwd.value);

    if (success) {
      router.push('/');
    } else {
      // Pull the specific error message from the composable's state
      error.value = state.error || 'Invalid credentials. Please try again.';
    }
  } catch (e) {
...
.shape-1 {
  width: 350px;
  height: 350px;
  background: radial-gradient(circle, #2e7d3233, transparent 70%);
  top: -100px;
  right: -80px;
}
.shape-2 {
  width: 250px;
  height: 250px;
  background: radial-gradient(circle, #81c78422, transparent 70%);
  bottom: -60px;
  left: -60px;
}
.shape-3 {
